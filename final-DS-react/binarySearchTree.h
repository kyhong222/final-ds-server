...
public:
	...
	*/
	...
	void ResetList() { pointer = root; }

	MySortedLinkedList<ItemType> GetItemList();

private:
	Node<ItemType>* root;		// Node 타입의 root
	Node<ItemType>* pointer;
};
...
template<class ItemType>
void Update(Node<ItemType>* root, ItemType& item, bool& found) {
	if (root == NULL) {
		return;
	}
	if (root->data == item) {
	...
	Update(root->left, item, found);
	Update(root->right, item, found);
}

// 아이템을 업데이트
template<class ItemType>
void BinarySearchTree<ItemType>::UpdateItem(ItemType item) {
	bool found = false;
	Update(root, item, found);
}
...
template<class ItemType>
void AddOnList(Node<ItemType>* root, MySortedLinkedList<ItemType>& list) {
	if (root != NULL) {
		list.Add(root->data);
		AddOnList(root->left, list);
		AddOnList(root->right, list);
	}
}

template<class ItemType>
MySortedLinkedList<ItemType> BinarySearchTree<ItemType>::GetItemList() {
	MySortedLinkedList<ItemType> list;
	AddOnList(root, list);
	return list;
}
